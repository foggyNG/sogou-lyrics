...
class Baidu(threading.Thread):
	...
	def run(self):
		...
		url = 'http://www.baidu.com/s?wd=%s+-+%s+filetype%%3Alrc' % (title_token, artist_token)
		...
		try:
			...
			pattern = re.compile(r'href="(?P<url>[^"]+?)"  target="_blank" ><font size="3">')
			urls = []
			for line in cache:
...
if __name__ == '__main__':
	...
	if len(args) != 0:
		...
		engine = Baidu(options.artist, options.title, console_receiver, options.timeout, options.max)
		engine.start()
		engine.join()
